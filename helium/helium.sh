...
function header() {
	...
	echo -e $WHITE" by NETWORK FREE" $NOCOLOR
}
...
function DNSOption() {
	...
	echo
        heliumStatus
        echo
	echo -e " ${WHITE}Change NF DNS${NOCOLOR}"
	echo -e " [1] Google
 [2] Cloudflare
...
function changeDNS() {
	echo -e -n " Changing to ${PROVIDER} NF DNS..."
...
function reinstall() {
	...
	echo -e -n " Reinstalling NF Helium..."
	sleep 2
	[[ ! -e /etc/dnsmasq ]] && mkdir -p /etc/dnsmasq
	echo "nameserver 1.1.1.1" >/etc/resolv.conf
	echo
...
function uninstall() {
	...
	echo -e -n " Uninstalling NF Helium..."
